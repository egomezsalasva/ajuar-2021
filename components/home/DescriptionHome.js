...
const DescriptionContainer = styled.div`
  ...
  max-width: calc(
    100vw - ${pxToVW.vw30px} - ${pxToVW.vw30px} - ${pxToVW.vw220px}
  );
  @media (max-width: ${breakpoints.laptopDesign}) {
    margin: 30px 30px 20px;
    max-width: calc(
      100vw - 30px - 30px - 220px
    ); //full screen - margins - menuContainerSize
  }
`;
  ...
  return (
    <DescriptionContainer>
      <Line>
        ...
        <br />
        PARA EL "SÍ <span className="emoji">💍</span> QUIERO" DE TU CLIENTE{" "}
        <span className="supScript">(AMOR)</span>.
      </Line>
    </DescriptionContainer>
  );
}
